...
def chooseSide(): # Called from many functions to check if the player has chosen a side for this game.
    ...
    mute()
    playerside = int(me.getGlobalVariable('playerside'))
    if playerside == 0:
        ...
        if me.hasInvertedTable():
            ...
            playerside = 1
    me.setGlobalVariable('playerside',str(playerside))
    debugNotify("<<< chooseSide() returned {}".format(playerside)) #Debug
...
def positionOffset(card, oldx, oldy):
    if me.hasInvertedTable():
        x = (-1 * oldx) - (cwidth(card,0) -3)
        y = (-1 * oldy) - cheight(card,0)
        return [x,y]
