...
def ofwhom(Autoscript, controller = me): 
      ...
      if re.search(r'o[fn]Opponent', Autoscript):
             if len(getPlayers()) > 1:
                     ...
                     if debugVerbosity >= 1: delayed_whisper("There's no valid Opponents! Selecting myself.")
                     targetPL = me
      else: 
             if len(getPlayers()) > 1:
...
def getCommitted(card):
      ...
      for key,value in commList.iteritems():
             if card._id in value: return key
      return none
      ...
      try:
             ...
             if Automations['Placement']:
                     ...
                     if card.Type == 'Character': # For now we only place Units
                            unitAmount = len([c for c in table if c.Type == 'Character' and c.controller == me and c.highlight != UnpaidColor and c.highlight != DummyColor and c.orientation != Rot270]) - 1 # we reduce by 1, because it will always count the unit we're currently putting in the game
                            ...
                            if hostType:
                                  ...
                                  supportAmount = len([c for c in table if c.Type == 'Support' and c.controller == me and c.orientation != Rot270 and not hasSubType(c,'Attachment.')])
...
def freeCardPlacement(card): # A function which stores a unit's position when it leaves play, so that it can be re-used by a different unit
      if Automations['Placement'] and (card.Type == 'Character' or card.Type == "Support") and card.orientation != Rot270:
             if card.owner == me and card.highlight != DummyColor and card.highlight != UnpaidColor :
                     if card.Type == 'Character':
                            freePositions = eval(me.getGlobalVariable('freePositions')) # We store the currently released position
                            freePositions.append(card.position)
                            me.setGlobalVariable('freePositions',str(freePositions))
                     elif card.Type == 'Support' and not hasSubType(card, 'Attachment.'):
...
def cardStatus(card):
      ...
      elif (card.type == "Story"): cS = "Story"
      elif (card.type == "Conspiracy"): cS = "Conspiracy"
...
def getIcons(card, type, printedOnly = False, verbose = False):
      ...
      count = card.Icons.count(iconSearch)
      cardCount = count
      if printedOnly: return count
             ...
             for cTuple in iconModifiers:
                     ...
                     if iconSearch.group(4) == 'All' or checkCardRestrictions(gatherCardProperties(card), prepareRestrictions(autoS, seek = 'reduce')):
...
def updateScores():
    ...
    for card in me.piles['ScoringPile']:
         if card.Type == 'Story' or card.Type == 'Conspiracy':  count += 1
...
def loadDeck(player,groups):    
      if player == me:
             chooseSide()
             if  len(me.piles['Deck']) >= 50 and (int(me.getGlobalVariable('playerside')) == -1 or len(shared.piles['Story Deck']) > 0 or confirm("Do you want to set up the table now?  Select No only if you want to load a custom Story Deck.")): intSetup(table)
#-------------------------------------------
#  Other Stuff
#-------------------------------------------
...
def switchDamageAutomation(group, x=0, y=0):
     ...
     if debugVerbosity >= 1: notify(">>> switchDamageAutomation(){}".format(extraASDebug())) #Debug
...
def switchPaymentAutomation(group, x=0, y=0):
     ...
     if debugVerbosity >= 1: notify(">>> switchPaymentAutomation(){}".format(extraASDebug())) #Debug
